...
{
    public class ArtistsControllerTests : IClassFixture<InMemoryApplicationFactory<Startup>>
    {
        private readonly InMemoryApplicationFactory<Startup> _factory;

        public ArtistsControllerTests(InMemoryApplicationFactory<Startup> factory)
        {
            _factory = factory;
        }
        ...
        [Theory]
        [InlineData("/api/artists/?pageSize=1&pageIndex=0", 1, 0)]
        ...
        {
            var client = _factory.CreateClient();
            var response = await client.GetAsync($"/api/artists/{request.ArtistId}");
        ...
        public async Task Add_should_create_new_artist()
        {
            ...
            var httpContent = new StringContent(JsonConvert.SerializeObject(addArtistRequest), Encoding.UTF8,
                "application/json");
            var response = await client.PostAsync("/api/artists", httpContent);
            ...
            responseHeader.ToString().ShouldContain("/api/artists/");
        }
    }
}
